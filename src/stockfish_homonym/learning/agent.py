...
from stockfish_homonym.learning.loading import Batch, MAGIC_PAD_VAL
from stockfish_homonym.learning.nets.tstep_encoders import TstepEncoder
from stockfish_homonym.learning.nets.traj_encoders import TrajEncoder
...
class BaseAgent(nn.Module, abc.ABC):
    ...
    Args:
        ...
        rl2_space: A gymnasium space that is automatically generated by
            :py:class:`~stockfish_homonym.learning.envs.amago_env.AMAGOEnv` to represent the shape of extra
        ...
        tstep_encoder_type: Type of :py:class:`~stockfish_homonym.learning.nets.tstep_encoders.TstepEncoder` to use.
        traj_encoder_type: Type of :py:class:`~stockfish_homonym.learning.nets.traj_encoders.TrajEncoder` to use.
    """
...
class Agent(BaseAgent):
    ...
    Args:
        obs_space: Environment observation space (for creating input layers).
        rl2_space: A gymnasium space that is automatically generated by :py:class:`~stockfish_homonym.learning.envs.amago_env.AMAGOEnv`
            to represent the shape of extra input features for the previous action and reward.
        action_space: Environment action space (for creating output layers).
        max_seq_len: Maximum context length of the policy (in timesteps).
        tstep_encoder_type: Type of :py:class:`~stockfish_homonym.learning.nets.tstep_encoders.TstepEncoder` to use. Initialized based
            on provided gym spaces.
        traj_encoder_type: Type of :py:class:`~stockfish_homonym.learning.nets.traj_encoders.TrajEncoder` to use. Initialized based on
            provided gym spaces.
    ...
    Keyword Args:
        ...
        fbc_filter_func: Function that takes seq of advantage estimates and outputs the regression
            weights. See :py:func:`~stockfish_homonym.learning.agent.binary_filter` or :py:func:`~stockfish_homonym.learning.agent.exp_filter`. Defaults to :py:func:`~stockfish_homonym.learning.agent.binary_filter`.
        popart: If True, use :py:class:`~stockfish_homonym.learning.nets.actor_critic.PopArtLayer` normalization for value network outputs. Defaults to True.
        use_target_actor: If True, use a target actor to sample actions used in TD targets.
            Defaults to True.
        use_multigamma: If True, train on multiple discount horizons (:py:class:`~stockfish_homonym.learning.agent.Multigammas`) in parallel. Defaults to True.
        n_step: N-step horizon for TD targets. n=1 gives standard 1-step Bellman targets.
            Higher values use :py:func:`~stockfish_homonym.learning.agent.nstep_return` to sum discounted
            rewards over ``n`` future steps before bootstrapping with Q(s_{t+n}).
            Defaults to 1.
        actor_type: Actor MLP head for producing action distributions. Defaults to :py:class:`~stockfish_homonym.learning.nets.actor_critic.Actor`.
        critic_type: Critic MLP head for producing Q-values. Defaults to :py:class:`~stockfish_homonym.learning.nets.actor_critic.NCritics`.
    ...
    def get_actions(
        ...
        AMAGOEnv gymnasium wrapper. See `stockfish_homonym.learning.experiment.Experiment.interact` for an example.
...
class MultiTaskAgent(Agent):
    ...
    1. Converts critic regression to classification of two-hot encoded labels representing
       bins spaced across a fixed range (see stockfish_homonym.learning.nets.actor_critic.NCriticsTwoHot). The
    ...
    follow the information listed in :py:class:`~stockfish_homonym.learning.agent.Agent`.
    """
