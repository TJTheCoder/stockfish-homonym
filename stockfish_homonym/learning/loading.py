...
from stockfish_homonym.learning.hindsight import Trajectory, Relabeler, FrozenTraj, NoOpRelabeler
...
class RLDataset(ABC, Dataset):
    """Abstract base class that defines what :py:class:`~stockfish_homonym.learning.experiment.Experiment` expects from its pytorch Dataset.
    ...
    def configure(
        ...
        Args:
            ...
            padded_sampling. Strategy for sampling from trajectories longer than max_seq_len.
                See :py:meth:`~stockfish_homonym.learning.loading.RLData.random_slice` or :py:class:`~stockfish_homonym.learning.experiment.Experiment`
    ...
    def ready_for_training(self) -> bool:
        ...
        If `False`, :py:class:`~stockfish_homonym.learning.experiment.Experiment`
    ...
    def on_end_of_collection(self, experiment) -> dict[str, Any]:
        """
        Callback for :py:class:`~stockfish_homonym.learning.experiment.Experiment` to call
...
class MixtureOfDatasets(RLDataset):
    ...
    Args:
        datasets: A list of :py:class:`~stockfish_homonym.learning.loading.RLDataset` objects.
    ...
    Note:
        Only samples from datasets that are currently :py:meth:`~stockfish_homonym.learning.loading.RLDataset.ready_for_training`. For example,
        a mixture of some custom offline dataset and the :py:class:`~stockfish_homonym.learning.loading.DiskTrajDataset` will only sample
        from the offline dataset until the :py:class:`~stockfish_homonym.learning.loading.DiskTrajDataset` has written trajectories to disk.
...
def load_traj_from_disk(path: str) -> Trajectory | FrozenTraj:
    ...
    Returns:
        A :py:class:`~stockfish_homonym.learning.hindsight.Trajectory` or :py:class:`~stockfish_homonym.learning.hindsight.FrozenTraj` object.
    """
...
class DiskTrajDataset(RLDataset):
    ...
    Args:
        ...
        relabeler: A function that edits/relabels trajectory data when loaded.
            (`stockfish_homonym.learning.hindsight.Relabeler`).
    """
